...
pub fn validate_after_shell_execution_response(value: &Value) -> ValidationResult {
...
mod tests {
    ...
    fn delegates_to_base_hook_response_contract() {
        assert!(validate_after_shell_execution_response(&json!({})).is_valid);
        assert!(!validate_after_shell_execution_response(&json!("not-an-object")).is_valid);
    }
}
