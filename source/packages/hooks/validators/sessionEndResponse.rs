...
pub fn validate_session_end_response(value: &Value) -> ValidationResult {
...
mod tests {
    ...
    fn delegates_to_base_hook_response_contract() {
        assert!(validate_session_end_response(&json!({})).is_valid);
        assert!(!validate_session_end_response(&json!("not-an-object")).is_valid);
    }
}
