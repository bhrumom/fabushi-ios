...
pub fn validate_before_tab_file_read_response(value: &Value) -> ValidationResult {
...
mod tests {
    ...
    #[test] fn validates_tab_read_permission_and_message() {
        assert!(validate_before_tab_file_read_response(&json!({"permission":"deny","user_message":"no"})).is_valid);
        assert!(!validate_before_tab_file_read_response(&json!({"permission":"ask"})).is_valid);
    }
}
