pub const MAX_SELF_SUMMARY_RETRIES: usize = 3;
pub const SELF_SUMMARY_NUM_TURNS: usize = 1_000;
pub const SELF_SUMMARY_CONTEXT_WINDOW_FRACTION: f64 = 0.9;

pub const SELF_SUMMARIZATION_PROMPT: &str = r#"<user_query>
<summary_request>
...
pub const ANTHROPIC_COMPACTION_INSTRUCTIONS: &str = r#"You have written a partial transcript for the initial task above. Write a summary for continuity in a future context. Include state, next steps, and learnings. Wrap your summary in <summary></summary> tags. IMPORTANT: Do not call any tools. Respond immediately with only the <summary> block"#;
pub const CLAUDE_CODE_COMPACTION_PROMPT: &str = r#"Your task is to create a detailed summary of the conversation so far, paying close attention to the user's explicit requests and your previous actions.
...
mod tests {
    ...
    #[test]
    fn numeric_contract_matches_reference() {
        assert_eq!(MAX_SELF_SUMMARY_RETRIES, 3);
        assert_eq!(SELF_SUMMARY_NUM_TURNS, 1_000);
        assert!((SELF_SUMMARY_CONTEXT_WINDOW_FRACTION - 0.9).abs() < f64::EPSILON);
    }

    #[test]
    fn prompt_contract_preserves_reference_text() {
        assert!(SELF_SUMMARIZATION_PROMPT.starts_with("<user_query>\n<summary_request>"));
        assert!(SELF_SUMMARIZATION_PROMPT.contains("DO NOT call any tools in your response."));
        assert!(SELF_SUMMARIZATION_PROMPT.ends_with("</summary_request>\n</user_query>"));
        assert!(ANTHROPIC_COMPACTION_INSTRUCTIONS.contains("<summary></summary>"));
        assert!(CLAUDE_CODE_COMPACTION_PROMPT.contains("All user messages"));
        assert!(CLAUDE_CODE_COMPACTION_PROMPT.contains("Do not start on tangential requests"));
    }
}
