...
| --- | --- | --- |
| R1 | pending | The pinned inventory is present at 2,046/2,046 rows, but ledger closure is incomplete: 1,506 mapped, 520 implemented, 20 reviewed N/A, 0 verified at the audited snapshot; `replaces_ios_path` is still unfilled and some implemented/N/A rows still lack test evidence. |
...
| R11 | pending | source/packages now has 174 implemented, 670 mapped and 8 reviewed N/A rows. Shipping Swift covers `agent-client/errors`, typed context/abort/logger/span semantics, context-injected metrics, and the complete prompt JSX/AST rendering responsibility with FabushiTests evidence; hooks-exec fail-closed denial policy is now compiled into the shipping mobile Host crate with Rust behavior tests; remaining package responsibilities still require implementation/evidence. |
