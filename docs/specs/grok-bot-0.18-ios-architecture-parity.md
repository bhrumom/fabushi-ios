...
| --- | --- | --- |
| R1 | pending | The pinned inventory is present at 2,046/2,046 rows, but ledger closure is incomplete: 1,505 mapped, 521 implemented, 20 reviewed N/A, 0 verified at the audited snapshot; `replaces_ios_path` is still unfilled and some implemented/N/A rows still lack test evidence. |
...
| R11 | pending | source/packages now has 175 implemented, 669 mapped and 8 reviewed N/A rows. Shipping Swift covers `agent-client/errors`, typed context/abort/logger/span semantics, context-injected metrics, and the complete prompt JSX/AST rendering responsibility with FabushiTests evidence; hooks-exec fail-closed denial policy is now compiled into the shipping mobile Host crate with Rust behavior tests; shared path-utils now owns worktree/realpath containment policy and is consumed by the shipping Host protected-path guard; remaining package responsibilities still require implementation/evidence. |
...
| R14 | pending | Scene/background/protected-data/relaunch recovery code and tests exist. Protected CI session transport is runner-carried and materialized as a 0600 file inside the app's current sandbox at launch. Run 35827812333 proved that transport and build path, then exposed a fresh-sandbox production bug: the auto-derived app-data workspace was warmed before it existed. The Host now creates only that generated workspace before provider warmup while explicit user workspaces remain fail-closed; packaged E2E and termination/relaunch still require successful exact-SHA evidence. |
