...
| R14 | pending | Scene/background/protected-data/relaunch recovery code and tests exist. Protected CI session transport is runner-carried and materialized as a 0600 file inside the app's current sandbox at launch, avoiding stale Simulator data-container paths after XCTest reinstalls the target app; packaged E2E and termination/relaunch still require successful exact-SHA evidence. |
